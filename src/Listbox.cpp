       ...
       : wxFrame(NULL, wxID_ANY, title, wxDefaultPosition, wxSize(270, 200))
{
  ...
  btnPanel = new MyListBoxPanel(panel);
...
MyListBoxPanel::MyListBoxPanel(wxPanel * parent)
       : wxPanel(parent, wxID_ANY)
{
  ...
  Connect(wxID_NEW, wxEVT_COMMAND_BUTTON_CLICKED, 
      wxCommandEventHandler(MyListBoxPanel::OnNew) );
  Connect(ID_RENAME, wxEVT_COMMAND_BUTTON_CLICKED, 
      wxCommandEventHandler(MyListBoxPanel::OnRename) );
  Connect(wxID_CLEAR, wxEVT_COMMAND_BUTTON_CLICKED, 
      wxCommandEventHandler(MyListBoxPanel::OnClear) );
  Connect(wxID_DELETE, wxEVT_COMMAND_BUTTON_CLICKED, 
      wxCommandEventHandler(MyListBoxPanel::OnDelete) );
...
void MyListBoxPanel::OnNew(wxCommandEvent& event) 
{
...
void MyListBoxPanel::OnClear(wxCommandEvent& event) 
{
  m_lb->Clear();
}

void MyListBoxPanel::OnRename(wxCommandEvent& event) 
{
  wxString text;
  wxString renamed;
...
void MyListBoxPanel::OnDelete(wxCommandEvent& event) 
{
  int sel = m_lb->GetSelection();
  if (sel != -1) {
      m_lb->Delete(sel);
